# 第 11 章 · Cache 与 Checkpoint

本章从第 10 章的网络执行代码继续往前走，在 `RDD.iterator(partition)`
...
- `checkpoint()` 只登记请求；后续 action 第一次读到该 RDD 时把分区写到本地临时目录。
- checkpoint 物化完成后，`dependencies()` 改为指向 `CheckpointRDD`，让调度器不再追原父血缘。
- `getComputeCount()` 只服务于本章 demo 和测试，用来观察重算成本。
- `Main` 里还放了一个一维梯度下降的小例子，用当前的 `map` 和 `reduce` 对比重复训练时有没有 cache。
