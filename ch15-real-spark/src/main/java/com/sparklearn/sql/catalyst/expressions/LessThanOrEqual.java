 ...
 * 小于等于比较。
 *
 * <p>对应 Spark 的 {@code LessThanOrEqual}（sql/catalyst/.../expressions/predicates.scala）。
 */
public record LessThanOrEqual(Expression left, Expression right) implements Expression {
    ...
    public Expression transform(ExpressionRule rule) {
        return rule.apply(new LessThanOrEqual(left.transform(rule), right.transform(rule)));
    }
