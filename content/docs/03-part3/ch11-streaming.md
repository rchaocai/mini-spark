...
前 10 章，你已经攒出一台分布式批处理引擎：

```mermaid
flowchart LR
    In(["一批静态数据<br/>（一次到齐）"]) --> P["分区"]
    P --> L["血缘"]
    L --> G["Stage / DAG"]
    G --> T["Task → Executor<br/>（分布在多台机器）"]
    T --> S["Shuffle<br/>跨分区搬数据"]
    S --> Out(["结果"])
    L -. 失败时沿血缘重算 .-> T
    S -. 反复读时短路 .-> C["cache / checkpoint"]
```

数据进来先分区，血缘记下每一步怎么来的，Stage / DAG 排出执行顺序，Task 发到多台机器上算，Shuffle 在分区之间搬数据；中途某个分区失败，就沿血缘把它重算回来，反复要读的分区用 cache / checkpoint 短路。

这套机构都默认同一件事：**数据是一次到齐的一批**。可现实里有大量数据是**持续到来**的——网站的点击、服务器的日志、传感器的读数。它们最值钱的时候，往往是刚刚到达的那一秒。
...
先把"流计算到底难在哪"想清楚，才能看懂接下来那个看似奇怪的做法。
...
常驻、有状态、来一条算一条的模型，让容错又贵又慢，还拿掉队者没办法。正是这个困局，催生了另一种思路。

## 11.2 离散化流：把流切成一串 RDD
...
这种做法听起来只是换了个切法，但它带来三个根本不同。

**首先，同一段数据，算几遍都是同一个结果。** 一段输入落定后就不再变，对它跑的又是一次确定性的批计算——结果完全由这段输入定死，跟在哪台机器上算、算第几次都没关系。11.1 里那种“两个副本得看到一致顺序”的同步协议，在这里没了用武之地：既然算两遍结果一样，根本不用复制两份。

**其次，数据之间谁依赖谁，是一张看得见的血缘图。** 常驻算子的状态藏在它自己内存里，是一坨外面看不见的黑箱；离散化流不然，每一段数据都明明白白记着“我是从哪几段变来的”，细到一个分区一个分区。

**最后，容错因此变得又便宜又快。** 哪个分区丢了，不用另写一套流式恢复协议，照着血缘把它重算出来就行，和第 8 章一模一样；而且依赖细到分区，重算还能**并行**——丢一个节点，它的那些分区散到集群里很多机器上同时算，不同时间段的独立步骤也能一起重跑。这是“来一条算一条”的常驻模型做不到的：它默认每个算子得串行地被喂输入，连复制都要费很大劲。11.1 里那种接不住的掉队者，到这里不过是“再多并行重算几个分区”罢了。
...
这里的 job，就是 `collect()`、`reduce()` 那种 action 一触发就跑起来的一整轮——把一直拖着没算的 RDD 真正算出来，切出 Stage、提交 Task、把结果收回来。**一个 action 对应一个 job**：你每写一次 `collect()`，背后就是提交了一个 job。

流计算没有另起一台引擎。它只是在外面加了一层“时间驱动器”：每隔一个 batch 间隔，把这段时间的数据收成 RDD，再自动提交一个 job 把它跑出来——就像你自己写了一次 `collect()`。
...
主程序先把这三个 RDD 装进一个队列，再当成“流”接进来；之后对它做的事，和第 1 章的 WordCount 几乎一样，只不过现在挂在“流”上：
...
开头那个 `lines`，就是一个 `Queue<RDD<String>>`，装着上面那三个 RDD。每个 RDD 都由 `sc.parallelize(数据, 分区数)` 造出来——第二个参数是分区数：前两个 batch 各有两条数据，切成 2 个分区；第三个只有一条，切成 1 个分区。接下来 `ssc.queueStream(lines)` 把这个队列接成一条 `DStream<String>`。`queueStream` 是 `StreamingContext` 上的方法，它有几个重载，Main 调的是只收一个队列的那个：
...
`<T>` 是占位类型——队列里装的是 `RDD<String>`，`T` 就是 `String`，返回的也就是 `DStream<String>`。第一个重载把 `queue` 往下传，再补两个默认值（`true` 表示每个 batch 取一个 RDD，第三个是一条空 RDD，留给队列空时兜底），交给第二个、参数最多的重载；那里 `new QueueInputDStream(...)` 才真正把队列包成一条 `DStream`——你拿到的 `DStream<String>` 就是它。
...
刚才那条流，从输入到输出都靠 `DStream` 串起来。把它和 RDD 并排放：
...
`DStream` 自己不存数据，它只记得一件事：给一个时间点 `Time`，怎么算出对应的 `RDD`。你给它一个 `Time`，它就返回一个 `RDD`；再给下一个 `Time`，又返回一个。
...
演示里那条 WordCount，一共有五个 `DStream` 节点——输入（`queueStream`）、`flatMap`、`map`、`reduceByKey`、输出（`foreachRDD`）。它们怎么连，一开始就定死了：不管时间走到哪，这五个节点、连法都不变，像一张模板。时间走到 `Time(1000)`，就照这张模板印一遍：每个节点各给出一个 `RDD`，五个 `RDD` 按同样的连法串成一条链。这条 RDD 链，就是这个 batch 要跑的血缘：
...
图里的虚线，把每个 `DStream` 节点和下面 RDD 图里对应的那个步骤连起来。换个时间点 `Time(2000)` 再印一遍，又出来一条 RDD 链：连法和上面一模一样，只是数据是新的。

上面的 DStream 图是张模板，每个 batch 照它印出一条 RDD 链。模板只有一张，实例每个 batch 一条，形状都一样。
...
表格只是声明。拿 `map` 这行当例子，看这个对应在代码里怎么兑现：

```java
// DStream.map：搭一个 MappedDStream 节点（惰性，不算数据）
public final <U> DStream<U> map(SerializableFunction<T, U> mapFunc) {
    return new MappedDStream<>(this, mapFunc);
}

// MappedDStream.compute：到某个时间点，对父 RDD 调同一个 map
