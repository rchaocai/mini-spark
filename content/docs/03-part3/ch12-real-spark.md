---
title: "第 12 章 · 致敬工业级 Spark"
weight: 4
date: 2026-07-21
tags: ["Spark 源码", "RDD", "DAGScheduler", "ShuffleMapTask", "运行行为对照", "Scala"]
summary: "把前 11 章亲手写的框架与真实 Apache Spark 源码并排放在一起：不只对照类签名，更对照一条 job 跑完、shuffle 数据交接、失败时两层恢复的运行行为。结构让你认得零件，行为让你看见同一台机器在转。"
---

# 第 12 章 · 致敬工业级 Spark

> 💻 本章完整代码：[GitHub 查看](https://github.com/rchaocai/mini-spark/tree/main/ch12-real-spark)
>
> 构建运行：`mvn -pl ch12-real-spark package`
>
> 运行示例：`java -Dfile.encoding=UTF-8 -cp ch12-real-spark/target/classes com.sparklearn.Main`
...
代码上，这一章和前 11 章几乎一样：把前 10 章完整执行内核快照搬到了 `ch12-real-spark/`（Streaming 在第 11 章独立模块），再补一个 `SparkSourceMapDemo` 打印静态对照表。打开本章目录，你看到的就是前 11 章的完整快照。

但看的角度变了。

```text
前 11 章：新概念 → 新代码 → 新 demo
本章：   让整机跑起来 → 对照运行行为
```
...
这个版本没有 Spark SQL，没有 Catalyst，没有 DataFrame API——正因为纯粹，它和你前 11 章写的代码对应关系才最清晰。
...
这张图就是本章第 12.4 节的目录：4.1 讲主流程（建 Stage 到出结果），4.2 讲 shuffle 数据交接，4.3 讲虚线那两层恢复，4.4 讲 cache / checkpoint 在哪一步短路。

## 12.4 运行行为对照

### 12.4.1 一条 action 怎么跑完
...
### 12.4.2 Shuffle 的运行时：Map 写盘 → Reduce 读盘
...
### 12.4.4 cache / checkpoint 的运行时：短路
...
对照要看版本：0.5 的 `dependencies` 是个 `@transient val`（算好一次、序列化时不写出，纯粹是序列化优化，没有 checkpoint）；从 0.7 起，真实 Spark 才把它改成 `final def`，并在里面加了 checkpoint 分支——checkpoint 后让依赖指向 `CheckpointRDD`，不再沿旧父血缘往上追。你这里的 `final` + `if (checkpointed) return List.of(new OneToOneDependency<>(checkpointRDD))`，和 0.7 几乎逐行一致（checkpoint 切断血缘的运行时对照见 12.4.4）。
...
真实 Spark 多出来的 `Aggregator`，就是 12.4.2 INFO 里讲的「把合并策略抽成对象」。

## 12.6 那工业级到底多了什么

如果运行行为一模一样，那「工业级」三个字贵在哪？

跑完上面的对照你会发现：每一步运行时轨迹，右边都有你左边的影子。但真实 Spark 的代码量远大于你前 11 章写的代码。多出来的部分，大致落在这些层：
...
> SchedulerBackend  ≈ 真实集群资源协议；本书没有这一层
...
你的数据源主要是 `ListRDD`。真实 Spark 还要读 HDFS、HBase、S3、Kafka……于是有 `HadoopRDD`、`NewHadoopRDD`，以及后来的 DataSource API。
...
本书停在 RDD 内核。真实 Spark 在内核之上还叠了整层产品面：
...
PySpark 尤其容易让人误会：你在 Python 里写 `df.groupBy(...).count()`，真正的 Stage 划分、Task 调度、shuffle 读写，仍然发生在 JVM 里的 Driver / Executor 上。Python 进程多半是在发指令、收结果；内核还是本章对照的那一套。

语言本身也一样。你用的是 Java 17；真实 Spark 内核主要是 Scala 写的，集合库、模式匹配、高阶函数让很多代码更短。更短，不等于逻辑不同。

把这些外衣剥掉，剩下的核心就是你前 11 章写过的东西。
...
├──────────────────────────────────────────┤
│  RDD / DAGScheduler / Task / Shuffle     │  执行内核 ← 本书
├──────────────────────────────────────────┤
...
工业级 Spark 贵在把每一层都补到能上生产；你前 11 章抓住的，是中间那块不会随版本换皮的内核。
