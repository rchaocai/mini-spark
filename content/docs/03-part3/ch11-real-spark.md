...
注意：上面这段真实 Spark 代码（0.5）**只有 cache，没有 checkpoint**——checkpoint 是 Spark 0.7 才引入的，0.5 还没有。所以 cache 这条短路，两边对得上：

```text
0.5 真实 Spark             你的实现
shouldCache → getOrCompute   shouldCache → 查 cache，命中就返回
否则 → compute               否则 → compute，结果按需存回
```

那 checkpoint 呢？真实 Spark 从 0.7 起才有，而且没塞进 `iterator()`，而是分到两处——切断血缘靠 `dependencies` 改指向 `CheckpointRDD`，读取靠 `computeOrReadCheckpoint` 短路：

```scala
// 0.7：checkpoint 后，依赖不再指向旧父 RDD，改指向 CheckpointRDD
final def dependencies: Seq[Dependency[_]] =
  checkpointRDD.map(r => List(new OneToOneDependency(r))).getOrElse { /* 否则走原本的依赖 */ }

// 0.7：已 checkpoint 就读 CheckpointRDD，否则 compute
...
你在第 10 章的做法，是把 checkpoint 也并进了同一个 `iterator()`：`checkpointed` 就读 checkpoint 文件，`dependencies()` 在 checkpoint 后返回空。两边目标一致——

```text
checkpoint 之后，不再沿旧父依赖往上追
读取时优先读 checkpoint，而不是 compute
```

只是切法不同：你让 `dependencies()` 返回空，真实 Spark 让它指向 `CheckpointRDD`；你直接读 checkpoint 文件，真实 Spark 读 `CheckpointRDD`。殊途同归，都是为了「切断血缘、给重算设上限」——这是第 10 章的设计目标，真实 Spark 在 0.7 用更完整的组件（`RDDCheckpointData`、`CheckpointRDD`）落地。
...
对照要看版本：0.5 的 `dependencies` 是个 `@transient val`（算好一次、序列化时不写出，纯粹是序列化优化，没有 checkpoint）；从 0.7 起，真实 Spark 才把它改成 `final def`，并在里面加了 checkpoint 分支——checkpoint 后让依赖指向 `CheckpointRDD`，不再沿旧父血缘往上追。你这里的 `final` + `if (checkpointed) return List.of()`，对应的正是 0.7 那一步演进（checkpoint 切断血缘的运行时对照见 11.4.4）。
