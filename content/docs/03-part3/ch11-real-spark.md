...
这一章把它们和真实 Apache Spark 的源码并排放在一起。但对照有两种看法：

```text
静态：贴出两边的类名、方法签名，一一指认
动态：让两边各跑一次同一个 job，看运行时的步骤对不对得上
```

第一种让你认得零件——「这个我写过」。第二种让你看见同一台机器在转——「它跑起来的样子，和我写的一模一样」。

这一章两种都做，但重心在第二种。因为签名对得上只能说明形似；一条 job 从提交到出结果、中间 shuffle 怎么交接、失败了怎么恢复——这些运行时步骤两边一致，才是「核心一模一样」的真正证据。
...
> **`val` 和 `final` 为什么这么常见？**
>
> Scala 里 `val` 是不可变引用，接近 Java 的 `final`。Spark 源码里几乎到处是 `val`。
>
> 这和第 8 章你已经体会过的路线一致：
>
> ```text
...
> ```
>
> 所以你前 10 章写的代码里大量用 `final`，不是风格偏好：不可变引用让重算结果幂等，血缘重算才安全。这是上面那条线落到字段层面的一环。

真实 Spark 的 `Stage` 用 `Option[ShuffleDependency]` 表示“可能有 shuffle 依赖，也可能没有”；你的 `Stage` 则同时有：
...
```text
有 shuffleDependency  → ShuffleMapStage
没有 shuffleDependency → ResultStage
```
...
运行时行为一样——「先看能不能短路，不能短路再 compute」：

```text
checkpoint 已完成  → 直接读 checkpoint 文件
否则 cache 命中    → 返回内存里的分区
否则              → 沿血缘 compute，结果按需存回
```

优先级也一致：

```text
checkpoint > cache > compute
```
...
「checkpoint 之后要切断血缘」这个目标两边一致：**checkpoint 之后，不再沿旧父依赖往上追。** 真实 Spark 把 `dependencies` 标成 `@transient val`（序列化时不写出，因为依赖链指向父 RDD，整条链一起序列化太重）；你的实现更朴素，整条 RDD 链 `implements Serializable` 直接丢进 `ObjectOutputStream`。教学上够用，生产级实现会在序列化上再走一步。
