...
第 1.5 节里说的逻辑回归和 PageRank，本质上也是同一批数据要反复读。这里把它缩成一条 RDD 链，就能直接看出：只要 cache 点命中，后面的 action 就不必再向前追。
...
checkpoint 也会先借这个入口改掉“从哪里读数据”。同时，它还要让调度器看到“这个 RDD 已经没有父依赖了”。依赖边界放到后面的 10.6 再拆。
...
同一个 `RDD` 类里还有 checkpoint 需要的 `checkpointed` 和 `checkpointDir`。这一节主要介绍 cache，checkpoint 会在 10.5 节单独展开。
...
接着选一个中间点作为缓存点。`traceUp(chain, 3)` 的意思是：从末端 `chain` 沿着父依赖往上走 3 层，拿到链路中间的那个 RDD。
...
| 场景 | 第一次 collect 后 | 第二次 collect 后 |
| --- | --- | --- |
| 不加 cache，源头 `source` | 3 | 6 |
| 不加 cache，中间点 `cachedPoint` | 3 | 6 |

第二次之所以变成 6，是因为第一次 3 个分区已经算过一次，第二次又把 3 个分区从头算了一遍。

再看加 cache 的版本。区别只有一行：在中间点 `cachedPoint` 上调用 `cache()`。
...
这就是 cache 在长血缘里真正省下来的成本：不是少执行一个 `map` 或一个 `filter`，而是让缓存点之前的整段血缘都不再被访问。
...
第 1.5 节里的逻辑回归，就是这种模式最典型的缩影。这里把训练过程收缩成一维梯度下降：
...
这两行里，真正的 action 不是 `checkpoint()`，而是后面触发计算的 `collect()`。
...
此时 `checkpointPoint` 还没有数据文件，`dependencies()` 也仍然能看到父 RDD。下一次 action 经过这个 RDD 时，`iterator(partition)` 才会真正起作用：
...
这一段就是 checkpoint 的物化过程：当前分区先按原来的血缘算出来，再写入 checkpoint 文件。`collect()` 会访问所有分区，所以它结束后，所有分区都已经写好。
...
这样写是为了把“checkpoint 物化后没有父依赖”这条规则收口到父类里，避免每个子类都重复判断一次。
...
注意前两个数字：调用 `checkpoint()` 以后，依赖数仍然是 1，`isCheckpointed` 仍然是 false。因为这时只是记录了 checkpoint 请求，还没有 action 真正把分区算出来。
