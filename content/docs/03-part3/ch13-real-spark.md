...
summary: "把前 12 章亲手搭出的执行底座与 Apache Spark 源码并排放在一起：不只对照类签名，更对照一条 job 跑完、shuffle 数据交接、失败时两层恢复的运行行为。先把容易混的 DStream 和 Structured Streaming 拆开，再看工业级 Spark 额外补了哪些层。"
---
...
比运行：跑本书的 job，再沿 Spark 源码查同一条路径
```
...
每条都先跑一遍本书的实现，再对照 Apache Spark 源码里的同一步。先把整机跑起来：
...
先看一张静态对照地图，再跑一段 `map → reduceByKey → collect` 的 pipeline，最后看失败与恢复的运行时。接下来的 13.4 节，就是把这些运行时拆开，和 Apache Spark 并排对照。
...
这张图就是 13.4 节的目录：13.4.1 讲主流程（建 Stage 到出结果），13.4.2 讲 shuffle 数据交接，13.4.3 讲虚线那两层恢复，13.4.4 讲 cache / checkpoint 在哪一步短路。
...
编码方式不同：这里的实现用一个 `boolean shuffleMap` 字段存死，Apache Spark 用 `shuffleDep != None` 现算。后者的好处是单一数据源（`shuffleDep` 既是依赖也是类型标记）；前者的好处是 `record` 字段更直白。信息相同，行为一致。
...
0.7 用 `storageLevel` 先判断要不要进入 cache 管理器：设了缓存级别就先走 `getOrCompute`（命中 cache 直接返回；没命中再去 `computeOrReadCheckpoint`，然后把结果存回去），没设缓存级别才直接进 `computeOrReadCheckpoint`——已 checkpoint 就读 `CheckpointRDD`，否则 compute。
...
flowchart TD
    ...
    P1 -- 是 --> Q1[物化：经 cache/compute 读出数据<br/>再写 checkpoint 文件]
    P1 -- 否 --> D1{shouldCache?}
...
flowchart TD
    ...
    B2 -- 是 --> C2[getOrCompute<br/>查 cache → 命中返回 / 未命中 computeOrReadCheckpoint+存]
...
```text
你的实现：checkpointed 优先；checkpointRequested 物化时会复用 iteratorWithoutCheckpoint，
         所以已有 cache 能被拿来写 checkpoint
Spark 0.7：先按 storageLevel 决定是否进入 CacheManager；
         cache 未命中后仍会落到 computeOrReadCheckpoint
```
...
>
> 为什么需要共享和有界驱逐？因为内存有限，而多个 Task、多个 RDD 实例副本、多个 Executor 都可能访问同一批分区。你这里的 `Map` 只跟着当前 RDD 对象走，在单机同一个 JVM 里够直观；到了集群里，它不能承担跨 Executor 可见性、块位置登记和驱逐策略。共享让缓存对同一 Executor 上的任务可见，有界驱逐让缓存不会无节制吃光内存。模式没变：
>
> ```text
> getOrCompute =
>   先查缓存
...
>
> 为什么 Spark 宁可多一轮 job 也要这么做？主要是**完整性**和**时机**：搭便车要求下游 action 把这个 RDD 的每个分区都读一遍才能物化完整，只读一部分就永远写不全；独立 job 强制遍历所有分区，保证写全。而且它在「用该 RDD 的 job 刚跑完」那一刻立刻触发，数据多半还在 cache，物化能直接复用、不必重算。再加上状态机管着进度，物化过程可见、可独立重试——这些在生产集群里是必须的。本书的搭便车省了那一轮 job，代价是默认「总会完整读取」：单机够用，生产集群不够稳。
...
你的 Java 版本在语言层面把两类依赖封住；Spark 0.5 没有用 `sealed`，而是用 `isShuffle` 在概念上分成窄依赖和宽依赖。`getParents(outputPartition)` 的签名几乎一字不差：它回答「子分区 0 的数据来自父分区哪几个」。
...
跑完上面的对照你会发现：每一步运行时轨迹，右边都有左边的影子。但 Apache Spark 的代码量远大于本书的实现。多出来的部分，大致落在这些层：

先把最容易混的两个名字拆开：

```text
第 11 章的 DStream
  = 把流切成一串 RDD，再按时间一批一批地跑

Structured Streaming
  = 把流看成一张不断追加的表，入口是 DataFrame / Dataset，
    由 Spark SQL 引擎增量执行
```

DStream 讲的是“怎么把流摊回 RDD”。它属于旧一代 Spark Streaming，现已是 legacy 路线，适合拿来讲底层。
Structured Streaming 讲的是“怎么把流塞进 SQL 引擎”。它是现在的推荐路线，适合拿来讲工业级优化。
它们都处理流，但站在不同楼层：

```text
DStream                 → RDD 视角，适合讲底层
Structured Streaming     → DataFrame / Dataset 视角，适合统一查询和优化
```

你这本书第 11 章写的，更像前者：每个 batch 都先变成一个 RDD，再沿血缘跑普通 job。这种写法今天不算 Spark 的主路，但特别适合把“流为什么能复用 RDD 的那套血缘、shuffle、checkpoint”讲透。
Structured Streaming 的优化点在后者：把整条流式查询交给 Spark SQL 一起优化，默认用 micro-batch 增量跑，配合 checkpoint / 写前日志做容错，并把 watermark、状态管理、流-批统一这些事收进框架里。对于支持的查询，还能切到更低延迟的连续模式。
...
> SchedulerBackend  ≈ 集群资源协议；本书没有这一层
...
本书的数据源主要是 `ListRDD`。Apache Spark 还要读 HDFS、HBase、S3、Kafka……于是有 `HadoopRDD`、`NewHadoopRDD`，以及后来的 DataSource API。

它们重写的仍然是 `compute(split)`。重写完以后，照样接 `map` / `filter` / `reduceByKey`。

**4. 更强的 Shuffle 与存储**

HashMap 分桶在单机内存里已经够用。数据大到内存装不下时，Apache Spark 会走到 SortShuffle、堆外内存、更紧凑的二进制格式。
...
Structured Streaming
  把流看成不断追加的无界表；
  入口是 DataFrame / Dataset，
  默认按 micro-batch 增量执行，
  统一接入 checkpoint、watermark、state 管理；
  这是 Spark 现在的推荐路线
