...
上一章结尾，我们手里握着两件法宝——"分块"和"并行"。要让计算最终在数据分区上铺开，代码里首先需要一个对象来代表这批数据，并说明每次该怎么访问它们。至于任务最终跑在哪台机器上，是调度层的事。

这一章，我们先造出这个代表的最小骨架。Spark 给它取了一个名字叫 RDD——眼下你可以把它当成一个普通的命名就好，重要的是它必须回答一个问题：数据怎么被访问。

> [!INFO]
> **RDD 是什么？**
>
> RDD 的全称是 Resilient Distributed Dataset（弹性分布式数据集）。这三个词刚好对应它的三层能力：
> - 弹性（Resilient）：节点故障时能自动恢复，靠的是血统和重算，后面的章节才会涉及。
> - 分布式（Distributed）：数据可以分散在多个分区上，第 1 章的“分块”就是在为它做准备。
> - 数据集（Dataset）：代表一批数据，并提供访问方式——这是本章要搭的骨架。
...
先从一个我们大概率用过却可能没认真想过的东西开始——`Iterator`。本节的几个小片段都放在 [`IteratorExamples.java`](https://github.com/rchaocai/mini-spark/tree/main/ch02-lazy-iterator/src/main/java/com/sparklearn/IteratorExamples.java) 里，可以直接运行。
...
## 2.2 把"待执行的计算"装进一个对象
### 为什么需要“延迟”？
在动手把计算装进对象之前，我们不妨先问一个问题：如果所有操作都立刻执行，会出什么问题？一个很熟悉的例子来自 Python 的 Pandas。早期的 Pandas 里，下面这样的链式操作：
```python
df[df['age'] > 25].groupby('city').mean()
```
在执行时，Pandas 会一步步“立刻做”：先产生一个过滤后的中间 DataFrame，再对它做分组聚合。每一步都实实在在地在内存里创建一份中间结果。当数据量不大时，这种 “急切求值” 的风格写起来很顺；可一旦处理的数据变大，或者操作链变长，这些中间副本就会让内存爆炸，也无法被提前优化（比如融合过滤和聚合，一次遍历完成）。

> [!INFO]
> **Pandas 是什么？**
>
> Pandas 是 Python 里处理表格数据最常用的库。你可以把它想象成“Python 世界的 Excel”——它能读 CSV、做筛选、分组汇总、画图，几行代码就能干完原本要写一堆循环的事。
>它的核心是一个叫 DataFrame 的东西，直白地说，就是一张有行有列的表格。每一列有名字，每一行有索引，你可以按列名取数据、按条件过滤行，就跟在 Excel 里操作一样顺手。
>
> **Pandas 后来做了什么？**
>
> 它开始引入“先别急着算，等我看看你总共要我干什么”的机制（比如 pandas.eval()、query()，以及正在演进的 Apache Arrow 后端）。简单说，就是把一连串操作攒在一起，最后一次性、用最优的方式算完。这和我们本章要讲的“延迟求值”，想的是同一件事。这也从侧面印证了：延迟求值不是 Spark 的专利，而是大规模数据处理的一条普遍规律。

这也正是 Spark 选择反其道而行之的根本原因。Spark 不希望每一步都马上执行，而是想先攒起一张 “要做的事”的清单，等到不得不产出结果时，再统一优化并执行。这样一来，原本需要多轮遍历的步骤可以被合并成一轮，中间结果也不必全部物化。

理解了这一点，你就会明白：我们要的是一种能把 “还没执行的计算” 包装起来的能力。下面要引入的 Supplier，就是 Java 里实现这种延迟的最简单工具。
...
`Supplier` 就是一个**还没执行的计算**。它不存结果，只存"怎么算"。
...
运行这段代码，控制台输出如下。`System.currentTimeMillis()`用于获取当前的时间戳，其每次运行都会不同：
...
构造 `Deferred` 时什么都没发生，第一次 `get()` 才触发计算；第二次 `get()` 获取的时间戳跟第一次相同，说明值已经被缓存，不再重复计算。从这个示例可以体会**延迟**的作用。

Spark 的 RDD 比 `Deferred` 走得更远：这里的 `Deferred` 会缓存第一次计算结果，而 RDD 不会缓存数据；每次需要数据时，它都会重新生成一个迭代器。眼下你只要先感受"构造一个东西 ≠ 马上执行"就够了。`Deferred` 的作用是让你感受‘延迟执行’本身，而真实 RDD 每次 `compute()` 都重新生成迭代器，是为了支持数据被重复消费且不占用额外内存，二者的目标不同。

## 2.3 RDD 的雏形：一个最小约定

有了 `Iterator` 和 `Supplier`，我们已经把“访问方式”和“延迟执行”这两件事都摸了一遍。现在可以把它们拼起来，造出 RDD 的第一个骨架。

但在看代码之前，先用一张图把第 1 章和第 2 章串起来。左侧是第 1 章代码被打包发往数据所在的节点的直觉；右侧是本章要搭建的结构——代码到达节点后，会交给一个叫 RDD 的东西，由它回答“我知道怎么访问这些数据”。

![RDD 作为数据代表的概念图](/mini-spark/images/ch02/rdd-data-representative.png)

*图 2-1：RDD 描述一批数据及其访问方式；真正运行时，任务会在具体分区上执行计算逻辑。*

不要把 RDD 想得很复杂。现在它只回答一个问题：如果有人要读数据，你能不能给他一个 `Iterator`？
...
这个定义看起来近乎"什么都没有"，但它正好抓住了本章的关键：RDD 先不急着保存一整份数据，也不急着算出结果；它先保存一张"怎么访问数据"的配方。
...
这段代码每次调用 `compute()`，也会创建一个新的 `Iterator`。所以关键区别不是"直接 `data.iterator()` 不行"。关键区别在于`Supplier` 把"怎么创建 Iterator"这件事抽象成一个字段。也就是说，`ListRDD` 保存的不是某个固定写死的读取动作，而是一个更一般的东西：

```java
Supplier<Iterator<T>>
```

它表达的是一句话：等 `compute()` 被调用时，你能不能给我一个新的 `Iterator`？今天这个 `Supplier` 是：

```java
// 最朴素的情况：数据已经在内存里，直接拿迭代器就行
() -> data.iterator()
```
...
```java
// 数据不在内存，在磁盘上：Supplier 封装的是"打开文件、逐行读取"
...
```java
// 数据被切成了多个分区，读哪个由 partitionId 决定——这是 RDD 走向分布式时的核心形状
() -> readFromPartition(partitionId)
```

就单单`ListRDD` 而言，直接保存 `List<T> data` 并在 `compute()` 中 `return data.iterator()` 也无可厚非。但是`Supplier` 让“数据从哪来”变成一个可替换的细节。数据来自内存、磁盘还是某个分片，RDD 根本不关心——它只认一件事：调用 `Supplier.get()`，拿到一个 `Iterator`。RDD 保存的是**生成数据流的方法**，而不是绑定某个具体容器上。

再换个角度看：`ListRDD` 不保存一个已经创建好的 `Iterator`，而是保存一个**创建 Iterator 的方法**。原因很简单：`Iterator` 是一次性的，走到末尾就没了；但"创建 Iterator 的方法"可以反复调用。每次调用 `compute()`，都能重新打开一扇从头开始的门。所以 `ListRDD extends RDD<T>` 的意思很朴素：`ListRDD` 遵守 RDD 的约定，它能在 `compute()` 被调用时给出一个 `Iterator<T>`。
...
同一个 `ListRDD` 可以反复消费，每次都是独立的。这里先记住一个事实：**RDD 本身不是那个已经走到末尾的迭代器；RDD 是能重新生成迭代器的对象。**
