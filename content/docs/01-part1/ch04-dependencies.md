...
summary: "给 RDD 装上正式骨架：分区、依赖和按分区计算。把原先无名的 parent 字段命名为 Dependency，并通过 OneToOneDependency 看清血缘链如何只记录配方、不保存数据。"
---
...
这个 `parent` 串起了两层 RDD。之前我们只是把它当作一个普通字段使用，还没有正式回答：

> 一个 RDD 和它的父 RDD 到底是什么关系？

这一章，我们给这段关系命名：**Dependency（依赖）**。有了依赖，沿着依赖一路往回走，就得到一条完整的血缘（Lineage）链。

同时，前面埋下的“分块计算”也要正式进入代码。`compute()` 不再只是“给我整条数据流”，而是变成“请计算第几个分区”。

## 4.1 把“第几块”落成 Partition

从 WordCount 的例子里，我们已经看到了两个朴素事实：数据太大时，必须切块；想并行时，也必须按块分工。

之前为了把注意力放在迭代器和惰性流水线上，暂时把“分块”藏了起来。现在要把它补回来。代码只有几行：
...
第二，`permits NarrowDependency, ShuffleDependency` 表示依赖只有两大类。先从父分区往子分区看，最容易抓住区别：

| 依赖类型 | 含义 | 本章状态 |
|---|---|---|
| `NarrowDependency` | 父 RDD 的每个分区，最多被子 RDD 的一个分区使用 | 正式实现 `OneToOneDependency` |
| `ShuffleDependency` | 父 RDD 的一个分区，可能被子 RDD 的多个分区使用，需要 shuffle | 先占位 |

换个方向，从子分区往父分区看也能说通：窄依赖下，子分区只需要少数固定的父分区，所以可以一路流水线计算；shuffle 依赖下，子分区要读取多个父分区写出的 shuffle 结果，中间必须先把数据按 key 重新分布。

Spark 源码里的 `NarrowDependency.getParents(outputPartition)` 用的是第二个方向：给定一个子分区，反查它依赖哪些父分区。两个方向说的是同一组父子分区关系，只是观察角度不同。

> [!INFO]
> **依赖类型是什么时候确定的？**
>
> 是在算子返回新 RDD 的时候就确定好的，不是调度器运行时临时猜出来的。
>
> 比如 `map`、`filter`、`flatMap` 不改变分区结构，所以它们构造出来的 RDD 会把依赖写成一对一窄依赖：
>
> ```java
> this.dependencies = List.of(new OneToOneDependency<>(parent));
> ```
>
> 需要按 key 重新分布数据的算子，会构造带 shuffle 语义的 RDD，并把依赖写成 `ShuffleDependency`。调度器后面只是读取这份已经写好的依赖结构：遇到 `ShuffleDependency` 就切开，遇到窄依赖就继续往父 RDD 追。

为什么要把这两类依赖在代码结构上写清楚？因为后面划分一组任务能不能放在一起执行时，核心判断就是：

> 遇到 shuffle 依赖就切开，窄依赖可以继续放在同一个 Stage 里。
...
这里多了一个看起来有点绕的 `iterator(partition)`。它现在只是直接调用 `compute(partition)`，好像没什么用。但以后加入缓存时，缓存逻辑会插在这里。
...
>
> 一个 RDD 的核心信息可以概括成五个问题：有哪些分区、依赖谁、每个分区怎么算、key-value 数据按什么规则分区、数据更适合在哪些机器上计算。本章先收敛前三个。剩下两个会等到 shuffle 和跨进程执行任务时再出现。
...
原来的 `parent` 字段还在，但它不再是一个无名关系。现在 `dependencies()` 会把它正式暴露成：
...
这正好接上前面提到的粗粒度变换。`map` 记录的不是“第 1 条记录改成什么、第 2 条记录改成什么”，而是“这个分区里的每个元素都应用同一个函数”。因为变换是粗粒度的，血缘才足够轻：保存一次函数定义，就能描述整个分区的重算方式。

> [!WARNING]
> **本章还不做容错重算**
>
> 血缘让“数据可以重算”成为可能，但本章只负责把血缘链记录出来。真正什么时候发现分区丢了、如何沿血缘重新计算，会在容错部分展开。
...
这里的 `for (Partition partition : partitions())` 是并行化的入口。现在它只是单线程循环；等它变成“为每个分区提交一个任务”，并行计算就会自然出现。

## 4.8 本章小结

这一章，我们把前面搭出的零件装成了更正式的 RDD 骨架。
...
到这里，第一部分的单机 RDD 内核已经成形。但它还没有真正并行。下一章，我们会把“遍历分区”改造成“提交多个 Task”，让每个分区可以由不同线程同时计算。再往后，shuffle 会让宽依赖真正落地，Stage 划分会用上本章建立的依赖结构，缓存和容错也会继续沿着 `iterator` 与血缘链往下长。
