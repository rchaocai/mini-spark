...
>
> 本章让读者写 `rdd.reduceByKey(...)`，底层返回的仍然是 `ShuffledRDD`。这是教学版 API 的一个小妥协：方法暂时挂在所有 `RDD` 上，Java 类型系统还不能阻止你在非键值对 RDD 上误用它。完整 Spark 会把这类方法放在 `PairRDD` / `JavaPairRDD` 上；我们现在不提前引入新抽象，只先把调用方向摆正。
>
> 另外，本章还没有调度器来拆分“先跑 Map 任务、再跑 Reduce 任务”，所以“通知上游 Map 分区先落盘”这件事，暂时放在 `ShuffledRDD.compute()` 里面做。
>
> 当前代码的真实执行顺序是：`collect(shuffled)` 先提交 2 个 Reduce 分区任务；第一个进入 `compute()` 的线程拿到锁，顺序遍历父 RDD 的 3 个分区，写出 `3 × 2` 个文件；随后 2 个 Reduce 分区任务分别读自己的文件。
>
> 真实跨进程执行时，物理过程会被拆开：Map 任务在持有输入分区的进程上写出 shuffle 文件，Reduce 任务再通过网络拉取这些文件并合并。逻辑 API 上仍然是 `rdd.reduceByKey(...)` 返回一个下游 `ShuffledRDD`，不是两个逻辑 RDD；只是执行层会分成两批任务。第 7 章会补“谁先跑、谁后跑”的调度边界，第 9 章再把本地读取换成跨进程网络读取。
...
这也解释了为什么真实系统会要求这类 reduce 函数满足结合律。先局部合并，再全局合并，结果应该和从头到尾一次性合并一样。本章用整数加法，因为它既满足结合律，也满足交换律；像减法这类和合并顺序强相关的函数，就不适合直接拿来做 `reduceByKey`。
...
真实 Spark 也必须先有 shuffle 输出，Reduce 端才能读取；区别是，工业级 Spark 可以重新调度丢失的 Map 任务，把缺失的 shuffle 输出再写出来。本章还没有容错和重试，所以会直接失败。
...
现在先不要急着给这条边界起更多名字。第 7 章会把这道边界正式用于调度，把一条长长的 RDD 血缘切成更大的执行单元。本章只需要把手感建立起来：只要同 key 数据散在多个分区里，就必须先重新分布；重新分布，就一定要有一个中间落点。
