---
title: "第 8 章 · 容错：从 Task 重试到 Stage 恢复"
weight: 4
date: 2026-07-17
...
上一章已经把一个 `reduceByKey(...).collect()` 作业切成了两个 Stage：

```text
ShuffleMapStage
  -> 提交一批 ShuffleMapTask
  -> 每个 Task 计算一个 Map 分区，并写出 shuffle 文件

ResultStage
  -> 提交一批 ResultTask
  -> 每个 Task 计算一个结果分区，并把结果交回 Driver
```

Stage 只是调度上的分组，不自己读写数据。真正读取迭代器、写 shuffle 文件、返回结果的，都是 Stage 里的 Task。

在这个例子里，`ResultStage` 的末端 RDD 是 `ShuffledRDD`，所以它提交的 `ResultTask` 会读取 shuffle 文件。但这不是 `ResultStage` 的定义；如果最终 RDD 只包含窄依赖，`ResultTask` 就不会读 shuffle。

顺序也已经固定：父 Stage 的所有 `ShuffleMapTask` 完成以后，才能提交下游 Stage 的 `ResultTask`。
...
因此在 `RDD` 上增加一个演示用 transformation：
...
它可以像 `map`、`filter` 一样写在链式调用里，但只用于本章测试和演示。真正承接这个算子的，是一个很薄的 `FaultyRDD`。它只负责把 `FaultyIterator` 放到指定分区外面：
...
完整实现见 [`RDD.java`](https://github.com/rchaocai/mini-spark/tree/main/ch08-fault-tolerance/src/main/java/com/sparklearn/RDD.java) 和 [`FaultyRDD.java`](https://github.com/rchaocai/mini-spark/tree/main/ch08-fault-tolerance/src/main/java/com/sparklearn/FaultyRDD.java)。
...
现在把上一节那条示例血缘真正跑起来。它在 Shuffle 之前调用了 `failOnNext(0, 2, remainingFailures)`，含义是：分区 0 的第二次 `next()` 失败一次。
...
> **为什么中断不能当成普通失败重试**
>
> `future.get()` 做的事，是让当前线程等 Task 的结果。这里的 `InterruptedException` 不是说 Task 里的数据算错了，而是说“正在等待结果的这个线程”收到了停止信号。
>
> 可以把它想成老师在教室里等学生交作业。学生作业写错了，是 `ExecutionException`；校长来通知老师立刻停止这节课，是 `InterruptedException`。后者不是某个学生的作业问题，继续让学生重写没有意义。
>
> Java 里还有一个细节：抛出 `InterruptedException` 时，中断标记会被清掉。所以代码先调用 `Thread.currentThread().interrupt()`，把这个“有人要求我停止”的标记补回去，再向外抛异常。这样外层代码如果也关心停止信号，仍然能看见它。
...
本章默认每个 Task 最多重试 3 次。

这里的“重试”不包括第一次执行。也就是说，默认情况下，一个 Task 最多会尝试 4 次：
...
也可以通过 `SparkContext` 显式设置重试次数。下面这个例子把重试次数改成 2，因此一个 Task 最多尝试 3 次：

```java
try (SparkContext sc = new SparkContext(
        2,  // 工作线程数
        2,  // 每个 Task 最多重试 2 次，总共最多尝试 3 次
        true)) {
    // ...
}
```
...
如果每个 action 都自己管理线程池和 Future，`collect()`、`count()`、`reduce()` 里都要各写一遍重试逻辑。现在所有 action 都先进入 `runJob(...)`，再由 `TaskScheduler.submitTasks(...)` 统一提交 Task，所以重试只需要改这一处。
...
如果一批 Task 里只有一个失败，调度器不应该把整批 Task 都重新提交。它应该做到：
...
如果这个边界不成立，后面讲“只重算失败分区”就没有基础。因为分区重算再正确，只要调度器每次都把整批 Task 重新提交，成功分区仍然会被无谓重跑。

这件事由 `submitTasks(...)` 的结构保证。

一开始，它仍然会把所有 Task 全部提交：
...
> **测试怎样验证这个边界**
>
> `FaultToleranceTest` 里有一个很小的测试：提交两个 `Callable`。第一个 Task 第一次执行时故意失败，第二次执行时返回 `10`；第二个 Task 第一次就返回 `20`。
>
> 测试最后检查两个计数器：
>
> ```java
...
> ```
>
> 第一个数字说明失败 Task 确实被重试了一次。第二个数字说明健康 Task 没有因为别的 Task 失败而重跑。完整测试见 [`FaultToleranceTest.java`](https://github.com/rchaocai/mini-spark/tree/main/ch08-fault-tolerance/src/test/java/com/sparklearn/FaultToleranceTest.java)。
...
不过，这里先停一下。调度器已经会重新提交失败 Task，但还没有回答一个更基础的问题：同一个分区再算一遍，为什么可以相信它会得到同样的结果？
...
本书的 `ListRDD` 为了避免复制，保存的是外部 List 的引用，并通过 `subList` 读取。因此 mini 实现还需要遵守一个明确约定：构造 `ListRDD` 以后，不再从外部修改原始 List。
...
为了稳定演示这条路径，本章新增 `MissingMapOutputRDD`。它不负责计算新数据，只负责在下游读取 shuffle 之前，模拟一个已经写好的 Map 输出文件丢失。

它以 `ShuffledRDD` 作为父 RDD：
...
`MissingMapOutputRDD` 不改变分区，也不改变数据内容。它的分区列表直接复用父 RDD：
...
真正的故障注入发生在 `compute(...)`。当目标 Reduce 分区第一次被计算时，它先删除一个 Map 输出文件，然后继续调用 `parent.iterator(partition)`，让 `ShuffledRDD` 按原来的方式读取 shuffle：
...
到这里，`FetchFailedException` 已经从 `TaskScheduler` 回到了 `DAGScheduler`。

`DAGScheduler` 不能继续重试当前 `ResultTask`。当前 Task 缺的是上游 Map 输出，应该回到产生这个输出的 `ShuffleMapStage`。
...
sequenceDiagram
    ...
    participant SR as ShuffledRDD
    participant TS as TaskScheduler
    participant D as DAGScheduler
    participant MS as ShuffleMapStage
    participant MT as ShuffleMapTask

    D->>RS: 准备提交 ResultStage
    RS->>TS: 提交本 Stage 的 ResultTask
    TS->>RT: call()
    RT->>SR: iterator(reduceId)
    SR--xRT: FetchFailedException(dependency, mapId, reduceId)
    RT--xTS: 当前 ResultTask 失败
    TS--xD: 交还 FetchFailedException
    D->>D: 根据 dependency 找到 ShuffleMapStage
    D->>MS: 定位 mapId 对应的 Map 分区
    D->>TS: 只提交这个 ShuffleMapTask
    TS->>MT: call()
    MT-->>MS: 重写该 Map 分区的 shuffle 输出
    D->>RS: Map 输出恢复，重新提交当前 ResultStage
    RS->>TS: 再次提交本 Stage 的 ResultTask
```

所以，DAGScheduler 提交 ResultStage 时，现在外面包了一层 Fetch 恢复循环：
...
这个循环先尝试提交当前 Stage。没有 Fetch 失败，就直接返回结果。捕获到 `FetchFailedException` 后，先检查恢复次数，再调用 `recoverMapOutput(...)`。

和普通 Task 重试一样，Fetch 恢复也必须有上限。文件如果被外部程序持续删除，或者读失败根本不是重算能解决的问题，调度器不能无限循环。

`recoverMapOutput(...)` 是这一节的核心。它把异常里的坐标翻译成一次上游 Map 分区重算：
...
```text
failure.dependency()
  -> 找到产生这次 shuffle 输出的 ShuffleMapStage
...
真正创建 Task 的地方在 `submitShuffleMapTask(...)`。它先从 RDD 的分区列表里找到 `mapId` 对应的分区，再提交一个 `ShuffleMapTask`：
...
注意，这里没有重新提交父 Stage 的全部 Map 分区。缺的是 `map_1_reduce_0`，系统重算的是 Map 分区 1：
...
`recoverMapOutput(...)` 返回以后，外层 `while (true)` 会再次提交当前 Stage。于是下游 `ResultTask` 再读到的，就是刚刚重写出来的 Map 输出。

本章的同步调度器会重新运行当前 Stage 的全部 `ResultTask`。它还没有像真实 Spark 那样保存每个 `ResultTask` 的完成状态，只做到：
