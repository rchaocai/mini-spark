 ...
 * 大于等于比较。
 *
 * <p>对应 Spark 的 {@code GreaterThanOrEqual}（sql/catalyst/.../expressions/predicates.scala）。
 */
public record GreaterThanOrEqual(Expression left, Expression right) implements Expression {
    ...
    public Expression transform(ExpressionRule rule) {
        return rule.apply(new GreaterThanOrEqual(left.transform(rule), right.transform(rule)));
    }
