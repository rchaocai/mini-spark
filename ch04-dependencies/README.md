...
- `Dependency.java`、`NarrowDependency.java`、`OneToOneDependency.java`：描述 RDD 之间的窄依赖关系；`ShuffleDependency.java` 目前只是占位壳，宽依赖的读写逻辑留给后续章节。
...
- `Main.java`：展示源头 RDD 的分区与依赖、打印血缘链、验证窄依赖，并演示 `collect`、`count`、`reduce`。
