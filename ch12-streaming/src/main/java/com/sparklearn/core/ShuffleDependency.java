...
import com.sparklearn.core.scheduler.MapOutputStatus;
import com.sparklearn.core.util.SerializableBinaryOperator;
...
public final class ShuffleDependency<K, V> implements Dependency<KeyValuePair<K, V>> {
    ...
    private final Map<Integer, MapOutputStatus> mapOutputStatuses =
    ...
    public ShuffleDependency(
        ...
        this.shuffleId = UUID.randomUUID().toString();
    ...
    public void registerMapOutput(MapOutputStatus status) {
        Objects.requireNonNull(status, "status");
        mapOutputStatuses.put(status.mapId(), status);
    }

    public MapOutputStatus mapOutputStatus(int mapId) {
        return mapOutputStatuses.get(mapId);
    }

    public void unregisterMapOutput(int mapId) {
        mapOutputStatuses.remove(mapId);
    }
}
